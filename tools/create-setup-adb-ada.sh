...
dest_directory=$base_directory/body_ada
template=dk8543jq-ada-setup.adb.template
destination=dk8543jq-setup.adb
date_time_utc=$(date -u "+%Y-%m-%dT%H:%M:%S")
library_name=$(cat $temp_directory/LIBRARY_NAME_ADA)
library_version=$(cat $temp_directory/LIBRARY_VERSION_ADA)
...
sed 's/@_LIBRARY_NAME_ADA_@/'$library_name'/'       | \
sed 's/@_LIBRARY_VERSION_ADA_@/'$library_version'/' | \

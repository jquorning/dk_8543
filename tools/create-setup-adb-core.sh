...
dest_directory=$base_directory/body_core
template=dk8543-core-setup.adb.template
destination=dk8543-setup.adb
date_time_utc=$(date -u "+%Y-%m-%dT%H:%M:%S")
library_name=$(cat $temp_directory/LIBRARY_NAME_CORE)
library_version=$(cat $temp_directory/LIBRARY_VERSION_CORE)
...
sed 's/@_LIBRARY_NAME_CORE_@/'$library_name'/'       | \
sed 's/@_LIBRARY_VERSION_CORE_@/'$library_version'/' | \
